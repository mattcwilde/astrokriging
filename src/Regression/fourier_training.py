...
fourier_model_cache = "proc_fourier_model.pkl"
plot_loc = "../Plots"

test_frac = 0.2
val_frac = 0.2 # Fraction of training data to use as validation for training hyperparams
n_alpha = 2 # Size of alpha grid search for ridge
...
if os.path.exists(os.path.join(data_loc,phys_cache)):
    ...
    with open(os.path.join(data_loc,phys_cache), 'rb') as handle:
        X_phys, y, names = pickle.load(handle)
else:
    raise NameError("%s not defined." % phys_cache)

# Now transform it was a neural network layer
# Pick k to be length of "training set"
print("Running fourier transformation...")
k_f = int(X_phys.shape[0]*(1.0-test_frac)*(1.0-val_frac))
#nn_k = 5000
X_f = bml.fourier_features(X_phys, k=k_f, verbose=False)
print(X_f.shape)
...
# Scale the data to have 0 mean, unit std
X_f = bml.scale_data(X_f)

# Split data into training set, testing set
X_train, X_test, y_train, y_test = train_test_split(X_f, y, test_size=test_frac,
                                                    random_state=seed)

# Make list of model defaults
models = [LinearRegression(),
          Ridge(random_state=seed, solver="sag"),
          RandomForestRegressor(random_state=seed)
          ]

# Do we need to train model's hyperparameters?
train_hyper = [False, False, False]
...
# Now cache...
print("Caching data at %s" % os.path.join(data_loc,nn_model_cache))
with open(os.path.join(data_loc,nn_model_cache), 'wb') as handle:
...
# Bootstrap!
print("Bootstrapping...")
lr_mean, lr_std = bu.bootstrap_error_estimate(lr, X_nn, y, nboots=nboots,
                                              seed=seed)
rr_mean, rr_std = bu.bootstrap_error_estimate(rr, X_nn, y, nboots=nboots,
                                              seed=seed)
...
cax = ax.scatter(X_phys[:,xind],X_phys[:,yind], c=rr_std, edgecolor="none", cmap="viridis")
cbar = fig.colorbar(cax)
cbar.set_label("Standard Deviation",rotation=270,labelpad=20)

# Format
ax.set_xlim(X_phys[:,xind].min(),X_phys[:,xind].max())
ax.set_ylim(X_phys[:,yind].min(),X_phys[:,yind].max())
ax.set_xlabel("b Inclination [degrees]")
ax.set_ylabel("c Inclination [degrees]")

fig.tight_layout()
fig.savefig(os.path.join(plot_loc,"nn_rr_inc_inc.pdf"))
