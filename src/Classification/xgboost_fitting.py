...
Thankfully, xgboost has sklearn API that looks like this for a classifier:
...
\midrule
0 &  XGBoost &  9.992007e-16 &     1.796933 &            1 &    0.947974 \\
\bottomrule
\end{tabular}

XGBClassifier(base_score=0.5, colsample_bylevel=1, colsample_bytree=0.5,
       gamma=0.3, learning_rate=0.01, max_delta_step=0, max_depth=8,
       min_child_weight=1, missing=None, n_estimators=2511, nthread=-1,
       objective='binary:logistic', reg_alpha=0, reg_lambda=0.0,
...
{} &  XGBoost &  Training MSE &  Testing MSE &  Training R2 &  Testing R2 \\
\midrule
0 &  XGBoost &  9.992007e-16 &     1.675987 &            1 &    0.951476 \\
\bottomrule
\end{tabular}
...
if run_fit:
    ...
    grid = GridSearchCV(xgbc, param_grid=params, cv=cv)
    ...
    ################################################################
    #
    # Step five: CV over learning rate, num estimators, fix everything
    # else again
    #
    ################################################################
...
# Evaluate Model
train_r2 = xgbc.score(X_train, y_train)
test_r2 = xgbc.score(X_test, y_test)

y_hat_train = xgbc.predict(X_train)
train_mse = log_loss(y_train, y_hat_train)

y_hat_test = xgbc.predict(X_test)
test_mse = log_loss(y_test, y_hat_test)
...
# Cache model? First echo model, then save
print(xgbc)
